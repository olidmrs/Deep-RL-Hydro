...
class HydroEnv(gym.Env):
    ...
    def __init__(
        ...
        self.action_space = gym.spaces.Discrete(l_max)
        self.state = 0
        self.last_i = 0
        self.current_t = 0
    ...
    def step(self, action : int) -> tuple[int, float, bool, bool, dict]:
        ...
        """        
        inflow = self.get_inflow(self.current_t)
    ...
    def get_inflow(self, t : int) -> int:
        ...
        match t:
            case 0:
                i = max(200 + 20 * np.random.normal(loc = 0, scale = 1), 0)
            case 1:
                i = max(self.last_i + 20 * np.random.normal(loc = 0, scale = 1), 0)
            case 2:
                i = max(3 * self.last_i + 20 * np.random.normal(loc = 0, scale = 1), 0)
            case _:
                return 0
        return int(i)