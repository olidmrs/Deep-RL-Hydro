...
class HydroEnv(gym.Env):
    ...
    class Continuous():
        def __init__(
            ...
            self.punition = punition
            self.deterministic_inflows = deterministic_inflows
            self.observation_space = gym.spaces.Tuple((gym.spaces.Box(l_min, l_max),gym.spaces.Box(l_min,l_max), gym.spaces.Discrete(t + 1)))
            self.action_space = gym.spaces.Box(l_min, l_max)
            self.state = 0
            self.last_i = 0
            self.current_t = 0
            self.inflow_cache = []
